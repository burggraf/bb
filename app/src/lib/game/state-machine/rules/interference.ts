/**
 * Catcher interference baserunning rules
 *
 * Rules:
 * - Batter is awarded 1B (no out recorded)
 * - Runner advancement follows walk/HBP rules (force advancement only)
 * - All runners advance one base if forced
 * - Bases loaded: Runner from 3B scores
 *
 * Note: Interference is treated exactly like a walk for baserunning purposes.
 */
...
export function handleCatcherInterference(
	...
	if (currentState.runners.first) {
		// Always forced when batter is awarded 1B
