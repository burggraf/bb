		...
		<section class="bg-zinc-900 rounded-lg p-4 sm:p-6">
			...
			{#if teamsDataDownloadState.isDownloaded}
				<div class="flex items-center gap-2 text-emerald-400 text-sm">
					...
					<span>Teams data loaded ({availableYears.length} seasons available)</span>
				</div>
			{:else if teamsDataDownloadState.isDownloading}
				<div class="flex items-center gap-2 text-blue-400 text-sm">
					...
					<span>Downloading {Math.round(teamsDataDownloadState.progress * 100)}%</span>
				</div>
			{:else if teamsDataDownloadState.error}
				<div class="flex items-center gap-2 text-red-400 text-sm">
					...
					<span>Error: {teamsDataDownloadState.error}</span>
				</div>
			{:else}
				<button
